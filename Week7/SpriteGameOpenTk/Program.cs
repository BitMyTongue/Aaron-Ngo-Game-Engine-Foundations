...
namespace OpenTK_Sprite_Animation
{
    public class SpriteAnimationGame : GameWindow
    {
        ...
        // Needed for Jumping:
        private float _playerY = 300f;        // extracted playerY position from this line --> Matrix4 model Matrix4.CreateTranslation(400, 300, 0)
        private float _verticalVelocity = 0f; // jump speed
        private float _gravity = -800f;       // pull down (negative)
        private float _jumpStrength = 400f;   // jump strength
        private float _groundY = 300f;        // ground level (same as start)

        private PlayerState _state = PlayerState.Idle; // player starts idle
        ...
        protected override void OnUpdateFrame(FrameEventArgs e)
        {
            ...
            if (_playerY > _groundY + 0.01f)        // if player is above ground, they're jumping
            {
                _state = PlayerState.Jumping;
            }
            else
            {
                if (dir == Direction.None)          // on ground and not moving = idle
                {
                ...
                else
                {
                    ...
                    if (sprinting)
                    {
                        _state = PlayerState.Sprinting;     // if shift held with direction
                    }
                    else
                    {
                        _state = PlayerState.Walking;       // if only direction is held
                    }
                }
            }

            // Update playerY position if jumping

            if (keyboard.IsKeyPressed(Keys.Up))
            {
                // Only jump if player is on the ground (0.01f margin of error for floats)
            ...
            _playerY += _verticalVelocity * (float)e.Time;

            // land
            if (_playerY < _groundY)
            {
                _playerY = _groundY;        // dont allow player to fall below ground level
                _verticalVelocity = 0f;     // vertical velocity back to 0
            }
            ...
            {
                deltaTime *= 1.5f;  // faster walking animation for "sprint"
            }

            // Animation update; when dir == None we *keep last frame visible*
            _character.Update(deltaTime, dir);
        }
        ...
        protected override void OnRenderFrame(FrameEventArgs e)
        {
            ...
            // Update model Y position for jumping
            int modelLoc = GL.GetUniformLocation(_shaderProgram, "model");
            Matrix4 model = Matrix4.CreateTranslation(400, _playerY, 0);        // 400 is the X pos center, _playerY
            GL.UniformMatrix4(modelLoc, false, ref model);
